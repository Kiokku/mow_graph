- ## 自我介绍
  collapsed:: true
	- 我是一位前端开发工程师，很高兴有机会在这里进行面试。我[[#green]]==毕业于西安交通大学==信息工程专业，并且在日本大阪有过学习经历，取得了JLPT N2级别的日语水平认证。在过去的工作中，我积累了丰富的前端开发经验。
	- 我熟练运用HTML、CSS和JavaScript进行前端开发，并且掌握了一些流行的前端框架和工具。[[#green]]==我熟练使用React作为Web开发框架，并且熟悉React状态管理库Redux，以及组件库Ant Design。==我也了解常用的前端工程化和构建工具，比如Webpack，能够完成Web应用打包和性能优化的工作。
	- 我对版本控制和代码托管使用Git非常熟练，[[#green]]==熟悉Git Flow的开发流程==。此外，我还有一定的TypeScript开发经验。
	- 在我的工作经历中，我曾在上海明品医学数据科技有限公司担任前端开发工程师，[[#green]]==参与了健康画像业务线的开发工作，使用React和Redux进行开发，并运用了多种第三方库实现各种功能。==我还参与了[[#green]]==医学课题研究支持平台的前端开发，使用Antd Form和JSON生成表单，并使用Echarts完成数据展示。==
	- 此外，我还参与了组件库的开发工作，使用dumi和TypeScript进行开发，并进行了JavaScript组件到TypeScript的[[#green]]==迁移工作==。我也参与了[[#green]]==春节集卡活动的前端开发==，使用umi和TypeScript进行开发，使用unstated-next进行状态管理。
