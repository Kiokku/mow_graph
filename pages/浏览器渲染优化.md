...
- ## React 时间调度实现
	- ### 基本原理
	  background-color:: pink
		- `rIC`：`requestIdleCallback`，维护一个队列，将在浏览器空闲时间（每一帧内）内执行。它属于 [Background Tasks API(opens in a new tab)](https://developer.mozilla.org/zh-CN/docs/Web/API/Background_Tasks_API)。
